...
export class Brc20PgStore extends BasePgStoreModule {
  ...
  async getStat(): Promise<DbBrc20Stat | undefined> {
    const result = await this.sql<DbBrc20Stat[]>`
    WITH tickers AS (
      SELECT COALESCE(COUNT(*), 0) AS total, COALESCE(SUM(tx_count), 0) AS tx_count FROM brc20_deploys
    ),
    holders AS (
    ...
    SELECT *, COALESCE((SELECT value FROM holders), 0) AS holders FROM tickers
    `;
  ...
  async getTokens(
    ...
    const tickerPrefixCondition =
      (args.ticker?.length ?? 1) > 1
        ? this.sql`d.ticker_lower IN(${args.ticker?.reduce((acc, val, i) => {
            if (i == 0) {
              return this.sql`LOWER(${val})`;
            } else {
              return this.sql`${acc},LOWER(${val})`;
            }
          }, this.sql``)})`
        : this.sqlOr(args.ticker?.map(t => this.sql`d.ticker_lower LIKE LOWER(${t}) || '%'`));
    const orderBy =
    ...
    const results = await this.sql<(DbBrc20Token & { total: number })[]>`
      ...
      tokens AS (
        ...
        ORDER BY ${orderBy}
        ${tickerPrefixCondition ? this.sql`` : this.sql`OFFSET ${args.offset} LIMIT ${args.limit}`}
      ),
      holders AS (
        SELECT COUNT(*) AS count, brc20_deploy_id
        FROM brc20_total_balances
        WHERE brc20_deploy_id IN (SELECT id FROM tokens) AND total_balance > 0
        GROUP BY brc20_deploy_id
      )
      SELECT t.*, COALESCE(h.count, 0) as holders 
      FROM tokens AS t
      LEFT JOIN holders AS h ON t.id = h.brc20_deploy_id
    `;
    return {
  ...
  async getMintFlow(
    args: { block_offset: string } & DbInscriptionIndexPaging
  ): Promise<DbPaginatedResult<DbBrc20MintStat> | undefined> {
    const offsetUnit = args.block_offset.at(-1);
    let blockRange;
    switch (offsetUnit) {
      case 'b':
        blockRange = this.sql`
          SELECT block_height - ${parseInt(
            args.block_offset.substring(-1, args.block_offset.length - 1)
          )} AS value FROM chain_tip
        `;
        break;
      case 'd':
        const now = new Date();
        const days = parseInt(args.block_offset.substring(-1, args.block_offset.length - 1));
        now.setDate(now.getDate() - days);
        blockRange = this.sql`
          SELECT MAX(block_height) AS value
          FROM locations
          WHERE
          timestamp <= to_timestamp(${now.getTime() / 1000})
        `;
        break;
      default:
        return;
    }
    const results = await this.sql<(DbBrc20MintStat & { total: number })[]>`
      WITH bestBlockHeight AS (${blockRange}),
      TOKENS AS (
          SELECT DISTINCT d.id, d.inscription_id, d.ticker, d.decimals, d.minted_supply, d.max
          FROM brc20_mints AS m
          INNER JOIN brc20_deploys AS d ON m.brc20_deploy_id = d.id
          WHERE m.block_height >= (SELECT value FROM bestBlockHeight)
      ),
      HOLDERS AS (
          SELECT COUNT(*) AS count, brc20_deploy_id
          FROM brc20_total_balances
      ...
      CONFIRMS AS (
          SELECT
              brc20_deploy_id,
              SUM(CASE WHEN block_height >= (SELECT value FROM bestBlockHeight) THEN 1 ELSE 0 END) AS mints
          FROM brc20_mints AS m
          WHERE block_height >= (SELECT value FROM bestBlockHeight)
          AND brc20_deploy_id IN (SELECT id FROM TOKENS)
          GROUP BY brc20_deploy_id
      )
      SELECT t.ticker, t.decimals, t.minted_supply, t.max, i.genesis_id,
            COALESCE(h.count, 0) AS holders,
            COALESCE(c.mints, 0) AS mint_count,
            COUNT(*) OVER () AS total
      FROM TOKENS AS t
      INNER JOIN inscriptions AS i ON t.inscription_id = i.id
      INNER JOIN HOLDERS AS h ON h.brc20_deploy_id = t.id
      INNER JOIN CONFIRMS AS c ON c.brc20_deploy_id = t.id
      ORDER BY c.mints DESC, holders DESC
      LIMIT ${args.limit}
      OFFSET ${args.offset}
    `;
    return {
